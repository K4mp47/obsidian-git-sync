...
private:
  ...
  void printTree(Node *node, int depth) const {
    ...
    std::cout << std::setw(4 * depth) << "" << node->key << node->color
              << std::endl;
  ...
  Node *buildBst(std::vector<int> arr, int inf, int sup, Node *x) {
    if (inf > sup)
      return nullptr;
    int med = (inf + sup) / 2;
    Node *r = new Node{arr[med], (std::random_device{}() % 2 == 0) ? 'n' : 'b',
...
public:
  ...
  Node *minimun() { return minimun(root); }
  ...
  void insert(int key) { root = insertAux(root, key); }
...
int heightFinder(Tree t, Node *x) { return finder(t.getRoot(), x->key); }
...
Tree copiaPari(Tree t) {
  auto buildBST = [](const std::vector<int> &arr, int inf, int sup, Node *x,
                     auto &self) -> Node * {
    if (inf > sup)
      return nullptr;
    int med = (inf + sup) / 2;
...
int partition(std::vector<int> &arr, int inf, int sup) {
  int pivot = arr[sup];
  int i = inf - 1;
...
void quickSort(std::vector<int> &arr, int inf, int sup) {
  if (inf < sup) {
    int pi = partition(arr, inf, sup);
    quickSort(arr, inf, pi - 1);
    quickSort(arr, pi + 1, sup);
  }
}
...
int main() {
  ...
  std::vector<int> arr4 = {20, 14, 6, 5, 8, 9, 7, 4, 2, 15, 18, 19, 16, 22, 21};
  ...
  // std::cout << alzettaNodi(t.getRoot(), finder(t.getRoot(), 16)) <<
  // std::endl;
